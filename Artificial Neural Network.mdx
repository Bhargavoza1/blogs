...
```
w is the weight being updated, $\alpha$ is the learning rate controlling the step size, and  $\frac{dE}{dw}$ is the gradient of the cost function with respect to the weight, guiding the update direction.
