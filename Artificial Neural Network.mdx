...
In general, most of the time we observe chain rule is calculated between cost function, the summation of weights and input features, and then computing the derivative with respect to the specific weight.
```math
...
we have to expand it further, considering the derivative with respect to the input of the activation function, denoted as $\frac{\partial C}{\partial S}$
