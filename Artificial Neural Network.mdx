...
date: '2023-12-14'
tags: ['deeplearning','neuralnetwork','ann','python','numpy','blog','project']
---
...
class MLP(Module):
    ...
    def __init__(self, input_size:int, output_size:int  , h_l_dimension:int = 10):
...
```
 Now, for the final term . $\frac{\partial y}{\partial w_2}$
```math
\begin{align*}
\frac{\partial y}{\partial w_2} = \frac{\partial ( x \times w_2+ b_2 ) }{\partial w_2} \\ \\ 
=  x
\end{align*}
```
 
What about bais? \
$\frac{\partial C}{\partial b} = \frac{\partial C}{\partial S} \times \frac{\partial S}{\partial y} \times \frac{\partial y}{\partial b} $\
$\frac{\partial y}{\partial b} = 1$\
We can take\
$\frac{\partial C}{\partial y} =\frac{\partial C}{\partial b} = $ output_error
...
In this section, we are breaking down the backpropagation process for the hidden layer with respect to $w_1$.\
The biggest question will arise. how we can find  $\frac{\partial C}{\partial R}$ (here R is ReLU activation function). And what is input from output layer?

Using the chain rule, we can expand this equation to.
```math
\frac{\partial C}{\partial R} = 
 \frac{\partial C}{\partial S} \times \frac{\partial S}{\partial y} \times \frac{\partial y}{\partial R}
```
With the help of this equation, we can find the input from output layer. \
We have $\frac{\partial C}{\partial S}$ and $\frac{\partial S}{\partial y}$, but we dont know what is this term $\frac{\partial y}{\partial R}$ for.
<CustomImage src="https://raw.githubusercontent.com/Bhargavoza1/blogs/main/images/ann/dydr.png"  alt="MLP" height={512} width={512} />
Now, we can find
```math
\begin{align*}
\frac{\partial y}{\partial R}  = \frac{\partial ( R \times w_2+ b_2 ) }{\partial R} \\ \\ 
=  w_2
\end{align*}
```
...
finding derivative of ReLU function.
```math
\begin{align*}
\frac{\partial R}{\partial H}  = \frac{\partial ( max(0,x) ) }{\partial H} \\ \\ 
=  \begin{cases}
  1 & \text{if } x > 0 \\
  0 & \text{if } x \leq 0
\end{cases}
...
```
 ...
 Now, for the final term . $\frac{\partial H}{\partial w_1}$
```math
\begin{align*}
\frac{\partial H}{\partial w_1} = \frac{\partial ( x \times w_1+ b_1 ) }{\partial w_1} \\ \\ 
=  x
\end{align*}
```

Writing every layer in reverse order. 
...
The whole process of moving information through the network (forward pass) and learning from mistakes (backward pass) makes up one round of training. 
We do this a bunch of times (iterations or epochs), thanks to the discovery of backpropagation. We can adjust the weights with this automated process. 

## Epoch
 - An epoch corresponds to a single, complete cycle through the entire training dataset while the model is being trained.
 - In each epoch, the model encounters and learns from every example in the training set exactly once.
 - Training a model for multiple epochs allows it to see the entire dataset multiple times, refining its parameters and improving performance.

## Batch Size
 - Batch size refers to the number of training examples utilized in one iteration.
 - In each iteration (or training step), the model processes a batch of data instead of the entire dataset.
 - Batch size influences the speed of training, memory requirements, and the stochastic nature of the optimization process.


Finally, we have implemented every step to train our neural network.
...
main.py
 ```python
    ...
    model = MLP(input_size=2 ,output_size=2 , h_l_dimension=20)
    train_neural_network(model , x_train, y_train, num_epochs=3000, learning_rate=0.1)
    predict_and_print_results(model , x_train, y_train )

```
**output**\
Epoch 2996/3000   Mean Squared Error: 0.000011\
Epoch 2997/3000   Mean Squared Error: 0.000011\
Epoch 2998/3000   Mean Squared Error: 0.000011\
Epoch 2999/3000   Mean Squared Error: 0.000011\
Epoch 3000/3000   Mean Squared Error: 0.000011\
[0.99599307 0.00387084]\
1st input for XOR: 0 2nd input for XOR: 0 Neural Network output is 0, actual output is: 0\
[0.00277158 0.99691068]\
1st input for XOR: 0 2nd input for XOR: 1 Neural Network output is 1, actual output is: 1\
[0.00241043 0.99837826]\
1st input for XOR: 1 2nd input for XOR: 0 Neural Network output is 1, actual output is: 1\
[0.99599521 0.00353148]\
1st input for XOR: 1 2nd input for XOR: 1 Neural Network output is 0, actual output is: 0

Our MLP able to solve XOR problem.

Full code is available on [GitHub](https://github.com/Bhargavoza1/simple-neural-network)  

If you are crazy and want to solve these equations on paper here is the link for you [Backpropagation Process](https://www.javatpoint.com/pytorch-backpropagation-process-in-deep-neural-network)
