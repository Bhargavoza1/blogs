...
What about bais? We can take 
$\frac{\partial E}{\partial y} =\frac{\partial E}{\partial b} = $ output_error
...
## Epoch
 ...
 - It represents a full cycle of the training data being presented to the neural network, and the network updating its weights and biases based on the error calculated on the entire dataset.
...
## Batch Size
 ...
 - In each iteration (or training step), the model processes a batch of training samples, computes the gradient of the loss function with respect to the model parameters, and updates the model's weights and biases.
