...
```
But, we have included the activation function between the cost function and the summation of weights and input features. This means we cannot directly perform $\frac{\partial C}{\partial y}$; 
...
```
Now, we proceed with the calculation of each term individually in order to derive equations that will assist us in writing Python code.

Partial differentiation of the cost function with respect to the activation function. basicaly it is derivative of the Mean Squared Error.
```math
\begin{align*}
\frac{\partial C}{\partial S} = \frac{\partial ( \frac{1}{n} \sum_{i=1}^{n} (S_i - \text{y}_i)^2) }{\partial S} \\ \\ 
= \frac{1}{n} \sum_{i=1}^{n} (S_i - \text{y}_i)^2
\end{align*}
```
